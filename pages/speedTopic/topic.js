...
Page({
	...
	onLoad: function (options) {
			...
			wx.setStorageSync('paperName','智能速练')
	//答题卡回跳
	...
	let serialNumber = this.data.serialNumber
		api.iniPastExamPages(param).then(res => {
			...
			if(this.data.topicCard[serialNumber - 1]&&this.data.topicCard[serialNumber - 1] !== ''){
				console.log(this.data.topicCard[serialNumber - 1].split(''))
				this.setData({
					answerArr:this.data.topicCard[serialNumber - 1].split('')
				},()=>{
					this.chkUserAnswer()
				})
			}
